...
vi.mock('@/actions/car-onboarding/nudge-preparation', () => ({
  nudgeCarOnboardingPreparation: vi.fn(),
}));
...
import { PUT } from '@/api/car-onboardings/[id]/preparation-nudge/route';
...
describe('PUT /api/car-onboardings/[id]/preparation-nudge', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    expect(nudgeCarOnboardingPreparation).not.toHaveBeenCalled();
  });
  ...
  it('returns the send result when admin triggers a nudge', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValueOnce({ user: mockAdmin } as never);
    vi.mocked(nudgeCarOnboardingPreparation).mockResolvedValueOnce({ sent: true });
    const response = await PUT({} as never, { params: Promise.resolve({ id: validId }) });
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ sent: true });
    expect(nudgeCarOnboardingPreparation).toHaveBeenCalledWith(validId, mockAdmin);
  });

  it('returns 403 when action rejects non-admin caller', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValueOnce({ user: mockAdmin } as never);
    vi.mocked(nudgeCarOnboardingPreparation).mockRejectedValueOnce(new CarOnboardingForbiddenError());
    const response = await PUT({} as never, { params: Promise.resolve({ id: validId }) });
    expect(response.status).toBe(403);
  });
});
