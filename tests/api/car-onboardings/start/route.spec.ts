...
vi.mock('@/actions/car-onboarding/start', () => ({
  startCarOnboarding: vi.fn(),
}));
...
import { PUT } from '@/api/car-onboardings/[id]/start/route';
...
describe('PUT /api/car-onboardings/[id]/start', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    expect(startCarOnboarding).not.toHaveBeenCalled();
  });
  ...
  it('returns 204 when admin starts onboarding', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValueOnce({ user: mockAdmin } as any);
    vi.mocked(startCarOnboarding).mockResolvedValueOnce(undefined);
    const response = await PUT({} as any, { params: Promise.resolve({ id: validId }) });
    expect(response.status).toBe(204);
    expect(startCarOnboarding).toHaveBeenCalledWith(validId, mockAdmin);
  });

  it('returns 400 when preparation is not ready', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValueOnce({ user: mockAdmin } as any);
    vi.mocked(startCarOnboarding).mockRejectedValueOnce(new CarOnboardingPreparationNotReadyError());
    const response = await PUT({} as any, { params: Promise.resolve({ id: validId }) });
  ...
  it('returns 403 when action rejects non-admin caller', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValueOnce({ user: mockAdmin } as any);
    vi.mocked(startCarOnboarding).mockRejectedValueOnce(new CarOnboardingForbiddenError());
    const response = await PUT({} as any, { params: Promise.resolve({ id: validId }) });
    expect(response.status).toBe(403);
  });
});
