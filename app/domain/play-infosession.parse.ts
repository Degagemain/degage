...
export const parsePlayInfosessionRegistrations = (value: string): { enrolled: number; maxRegistrations: number | null } => {
  const match = value.trim().match(REGISTRATIONS_PATTERN);
  if (!match) {
