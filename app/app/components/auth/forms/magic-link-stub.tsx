...
import { isMagicLinkEnabled } from '@/app/components/auth/lib/auth-features';

export function MagicLinkStub() {
  const t = useTranslations('auth');
  const router = useRouter();
  const { search } = useAuthRedirectTo();
  const enabled = isMagicLinkEnabled();
  ...
  return (
    <AuthCard title={t('magicLink')} description={t('featureNotConfigured')}>
