...
interface RoadAssistancePlansState {
  data: RoadAssistancePlan[];
  total: number;
  isLoading: boolean;
  error: string | null;
}
...
export default function RoadAssistancePlansPage() {
  const t = useTranslations('admin.roadAssistancePlans');
  const tCommon = useTranslations('admin.common');
  const [state, setState] = useState<RoadAssistancePlansState>({
    data: [],
    total: 0,
    isLoading: true,
    error: null,
  ...
  const [itemToDelete, setItemToDelete] = useState<RoadAssistancePlan | null>(null);
  ...
  const handleDeleteRequest = useCallback((item: RoadAssistancePlan) => {
  ...
  const fetchRoadAssistancePlans = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to fetch road assistance plans');
      }

      const result: Page<RoadAssistancePlan> = await response.json();
      setState({
        data: result.records,
        total: result.total,
        isLoading: false,
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    const response = await apiDelete(`/api/road-assistance-plans/${itemToDelete.id}`);
    if (response.ok) {
      toast.success(t('delete.success'));
      setItemToDelete(null);
      fetchRoadAssistancePlans();
    } else if (response.status === 409) {
      toast.error(t('delete.conflict'));
    } else {
      toast.error(t('delete.error'));
    }
  }, [itemToDelete, fetchRoadAssistancePlans, t]);
  ...
  const handleBulkDeleteItem = useCallback((id: string) => apiDelete(`/api/road-assistance-plans/${id}`), []);
  ...
  const handleUpsertRoadAssistancePlan = useCallback(async (record: RoadAssistancePlan): Promise<Response> => {
    if (record.id) {
      return apiPut(`/api/road-assistance-plans/${record.id}`, record);
    }
    return apiPost('/api/road-assistance-plans', { ...record, id: null });
  }, []);
    ...
    return (
      <div className="flex h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchRoadAssistancePlans} className="text-muted-foreground mt-2 text-sm underline hover:no-underline">
            {t('tryAgain')}
          </button>
        </div>
      </div>
  ...
  return (
    <>
          ...
          <DataTableToolbar
            ...
            leadingSlot={
              <Button variant="outline" size="sm" asChild>
                <Link href="/app/admin/road-assistance-plans/new">
            ...
            exportEndpoint="/api/road-assistance-plans/export"
      ...
      <BulkImportDialog<RoadAssistancePlan>
        open={bulkImportOpen}
        onOpenChange={setBulkImportOpen}
        getRecordLabel={(record) => record.name}
        upsertRecord={handleUpsertRoadAssistancePlan}
