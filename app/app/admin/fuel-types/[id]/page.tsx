...
export default function EditFuelTypePage() {
  ...
  const loadFuelType = useCallback(async () => {
    if (!id) {
  ...
  const handleSave = async (payload: FuelType) => {
    if (!id) return;
    setIsSaving(true);
    try {
      const response = await apiPut(`/api/fuel-types/${id}`, { ...payload, id });
  ...
  const handleDelete = async () => {
    if (!id) return;
    const response = await apiDelete(`/api/fuel-types/${id}`);
    if (response.ok) {
    ...
    return (
      <div className="flex h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={loadFuelType} className="text-muted-foreground mt-2 text-sm underline hover:no-underline">
            {t('tryAgain')}
          </button>
        </div>
      </div>
  ...
  return (
    <>
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="border-b px-3 md:px-4">
          <div className="flex h-14 items-center justify-start gap-2">
            <Button type="submit" form={FUEL_TYPE_FORM_ID} disabled={isLoading || isSaving || !fuelType} variant="outline" size="sm">
              <Save className="size-3.5" />
              {isSaving ? tCommon('status.saving') : tCommon('actions.save')}
            </Button>
            <Button variant="outline" size="sm" onClick={() => setIsDeleteDialogOpen(true)} disabled={isLoading || isSaving || !fuelType}>
        ...
        {isLoading ? (
          ...
          fuelType && <FuelTypeForm formId={FUEL_TYPE_FORM_ID} initialFuelType={fuelType} isSubmitting={isSaving} onSubmit={handleSave} />
        )}
      </div>
      ...
      <DeleteConfirmationDialog
        ...
        description={t('delete.description', { name: fuelType?.name ?? '' })}
