...
export default function EditInsurancePriceBenchmarkPage() {
  ...
  const [row, setRow] = useState<InsurancePriceBenchmark | null>(null);
  ...
  const loadRow = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/insurance-price-benchmarks/${id}`);
      if (!response.ok) {
        throw new Error(tCommon('feedback.loadError'));
      }
      const data: InsurancePriceBenchmark = await response.json();
      setRow(data);
    } catch (loadError) {
  ...
  const handleSave = async (payload: InsurancePriceBenchmark) => {
    if (!id) return;
    setIsSaving(true);
    try {
      const response = await apiPut(`/api/insurance-price-benchmarks/${id}`, { ...payload, id });
  ...
  const handleDelete = async () => {
    if (!id) return;
    const response = await apiDelete(`/api/insurance-price-benchmarks/${id}`);
    if (response.ok) {
  ...
  const deleteLabel = row != null ? String(row.year) : '';
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="border-b px-3 md:px-4">
          <div className="flex h-14 items-center justify-start gap-2">
            <Button type="submit" form={INSURANCE_PRICE_BENCHMARK_FORM_ID} disabled={isLoading || isSaving || !row} variant="outline" size="sm">
