...
export default function InsurersPage() {
  ...
  const handleDeleteRequest = useCallback((insurer: Insurer) => {
  ...
  const fetchInsurers = useCallback(async () => {
    ...
    try {
      ...
      const result: Page<Insurer> = await response.json();
      setState({
        data: result.records,
        total: result.total,
        isLoading: false,
  ...
  const handleDeleteConfirm = useCallback(async () => {
    if (!insurerToDelete?.id) return;

    const response = await apiDelete(`/api/insurers/${insurerToDelete.id}`);
  ...
  }, [insurerToDelete, fetchInsurers, t]);

  const selectedInsurers: BulkDeleteItem[] = useMemo(
    () =>
  ...
  const handleBulkDeleteItem = useCallback((id: string) => apiDelete(`/api/insurers/${id}`), []);
  ...
  const handleUpsertInsurer = useCallback(async (record: Insurer): Promise<Response> => {
    if (record.id) {
      return apiPut(`/api/insurers/${record.id}`, record);
    }
    return apiPost('/api/insurers', { ...record, id: null });
  }, []);
    ...
    return (
      <div className="flex h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchInsurers} className="text-muted-foreground mt-2 text-sm underline hover:no-underline">
            {t('tryAgain')}
          </button>
        </div>
      </div>
  ...
  return (
    <>
          ...
          <DataTableToolbar
            ...
            leadingSlot={
              <Button variant="outline" size="sm" asChild>
                <Link href="/app/admin/insurers/new">
            ...
            filterSlot={
              <BulkActionsButton count={selectedInsurers.length} label={t('bulkActions.label')}>
            ...
            exportEndpoint="/api/insurers/export"
      ...
      <DeleteConfirmationDialog
        open={insurerToDelete !== null}
        onOpenChange={(open) => !open && setInsurerToDelete(null)}
        onConfirm={handleDeleteConfirm}
        title={t('delete.title')}
        description={t('delete.description', { name: insurerToDelete?.name ?? '' })}
      ...
      <BulkDeleteDialog
        ...
        items={selectedInsurers}
        deleteItem={handleBulkDeleteItem}
        onComplete={handleBulkDeleteComplete}
        labels={{
          title: t('bulkDelete.title'),
          description: t('bulkDelete.description', { count: selectedInsurers.length }),
      ...
      <BulkImportDialog<Insurer>
        open={bulkImportOpen}
        onOpenChange={setBulkImportOpen}
        getRecordLabel={(record) => record.name}
        upsertRecord={handleUpsertInsurer}
