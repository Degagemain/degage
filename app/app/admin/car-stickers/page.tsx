...
export default function CarStickersPage() {
  const t = useTranslations('admin.carStickers');
  const tCommon = useTranslations('admin.common');
  const [state, setState] = useState<CarStickersState>({
    data: [],
    total: 0,
    isLoading: true,
    error: null,
  ...
  const [stickerToDelete, setStickerToDelete] = useState<CarSticker | null>(null);
  ...
  const handleDeleteRequest = useCallback((sticker: CarSticker) => {
  ...
  const columnLabels = useMemo(
    () => ({
      ...
      isAlwaysIncluded: t('columns.isAlwaysIncluded'),
  ...
  const fetchCarStickers = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to fetch car stickers');
      }

      const result: Page<CarSticker> = await response.json();
      setState({
        data: result.records,
        total: result.total,
        isLoading: false,
  ...
  const handleDeleteConfirm = useCallback(async () => {
    if (!stickerToDelete?.id) return;

    const response = await apiDelete(`/api/car-stickers/${stickerToDelete.id}`);
  ...
  }, [stickerToDelete, fetchCarStickers, t]);

  const selectedCarStickers: BulkDeleteItem[] = useMemo(
    () =>
  ...
  const handleBulkDeleteItem = useCallback((id: string) => apiDelete(`/api/car-stickers/${id}`), []);
  ...
  const handleUpsertCarSticker = useCallback(async (record: CarSticker): Promise<Response> => {
    if (record.id) {
      return apiPut(`/api/car-stickers/${record.id}`, record);
    }
    return apiPost('/api/car-stickers', { ...record, id: null });
  }, []);
    ...
    return (
      <div className="flex h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchCarStickers} className="text-muted-foreground mt-2 text-sm underline hover:no-underline">
            {t('tryAgain')}
          </button>
        </div>
      </div>
  ...
  return (
    <>
          ...
          <DataTableToolbar
            ...
            leadingSlot={
              <Button variant="outline" size="sm" asChild>
                <Link href="/app/admin/car-stickers/new">
            ...
            filterSlot={
              <BulkActionsButton count={selectedCarStickers.length} label={t('bulkActions.label')}>
            ...
            exportEndpoint="/api/car-stickers/export"
      ...
      <DeleteConfirmationDialog
        open={stickerToDelete !== null}
        onOpenChange={(open) => !open && setStickerToDelete(null)}
        onConfirm={handleDeleteConfirm}
        title={t('delete.title')}
        description={t('delete.description', { name: stickerToDelete?.name ?? '' })}
      ...
      <BulkDeleteDialog
        ...
        items={selectedCarStickers}
        deleteItem={handleBulkDeleteItem}
        onComplete={handleBulkDeleteComplete}
        labels={{
          title: t('bulkDelete.title'),
          description: t('bulkDelete.description', { count: selectedCarStickers.length }),
      ...
      <BulkImportDialog<CarSticker>
        open={bulkImportOpen}
        onOpenChange={setBulkImportOpen}
        getRecordLabel={(record) => record.name}
        upsertRecord={handleUpsertCarSticker}
