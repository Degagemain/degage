...
interface DocumentationGroupsState {
  data: DocumentationGroup[];
  total: number;
  isLoading: boolean;
  error: string | null;
}
...
export default function DocumentationGroupsPage() {
  const t = useTranslations('admin.documentationGroups');
  const tCommon = useTranslations('admin.common');
  const [state, setState] = useState<DocumentationGroupsState>({
    data: [],
    total: 0,
    isLoading: true,
    error: null,
  ...
  const [itemToDelete, setItemToDelete] = useState<DocumentationGroup | null>(null);
  ...
  const handleDeleteRequest = useCallback((item: DocumentationGroup) => {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/documentation-groups?${params.toString()}`);
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to fetch documentation groups');
      }
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    const response = await apiDelete(`/api/documentation-groups/${itemToDelete.id}`);
    if (response.ok) {
  ...
  const handleBulkDeleteItem = useCallback((id: string) => apiDelete(`/api/documentation-groups/${id}`), []);
  ...
  const handleUpsertGroup = useCallback(async (record: DocumentationGroup): Promise<Response> => {
    if (record.id) {
      return apiPut(`/api/documentation-groups/${record.id}`, record);
    }
    return apiPost('/api/documentation-groups', { ...record, id: null });
  }, []);
    ...
    return (
      <div className="flex h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <button type="button" onClick={() => void fetchGroups()} className="text-muted-foreground mt-2 text-sm underline hover:no-underline">
            {t('tryAgain')}
          </button>
        </div>
      </div>
  ...
  return (
    <>
          ...
          <DataTableToolbar
            ...
            leadingSlot={
              <Button variant="outline" size="sm" asChild>
                <Link href="/app/admin/documentation-groups/new">
      ...
      <BulkImportDialog<DocumentationGroup>
        open={bulkImportOpen}
        onOpenChange={setBulkImportOpen}
        getRecordLabel={(record) => record.name}
        upsertRecord={handleUpsertGroup}
