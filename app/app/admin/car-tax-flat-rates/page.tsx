...
interface CarTaxFlatRatesState {
  data: CarTaxFlatRate[];
  total: number;
  isLoading: boolean;
  error: string | null;
...
export default function CarTaxFlatRatesPage() {
  const t = useTranslations('admin.carTaxFlatRates');
  const [state, setState] = useState<CarTaxFlatRatesState>({
    data: [],
    total: 0,
    isLoading: true,
    error: null,
  ...
  const fetchRates = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to fetch car tax flat rates');
      }

      const result: Page<CarTaxFlatRate> = await response.json();
      setState({
        data: result.records,
        total: result.total,
        isLoading: false,
