...
export default function EditCarPriceEstimatePage() {
  ...
  const [row, setRow] = useState<CarPriceEstimate | null>(null);
  ...
  const loadRow = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/car-price-estimates/${id}`);
      if (!response.ok) {
        throw new Error(tCommon('feedback.loadError'));
      }
      const data: CarPriceEstimate = await response.json();
      setRow(data);
    } catch (loadError) {
  ...
  const handleSave = async (payload: CarPriceEstimate) => {
    if (!id) return;
    setIsSaving(true);
    try {
      const response = await apiPut(`/api/car-price-estimates/${id}`, { ...payload, id });
  ...
  const handleDelete = async () => {
    if (!id) return;
    const response = await apiDelete(`/api/car-price-estimates/${id}`);
    if (response.ok) {
  ...
  const deleteLabel = row != null ? `${row.carType.name} · ${row.year}` : '';
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="border-b px-3 md:px-4">
          <div className="flex h-14 items-center justify-start gap-2">
            <Button type="submit" form={CAR_PRICE_ESTIMATE_FORM_ID} disabled={isLoading || isSaving || !row} variant="outline" size="sm">
        ...
        {isLoading ? (
          ...
          row && (
            <CarPriceEstimateForm
              formId={CAR_PRICE_ESTIMATE_FORM_ID}
              initialCarPriceEstimate={row}
