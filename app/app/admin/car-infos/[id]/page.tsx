...
export default function EditCarInfoPage() {
  ...
  const [row, setRow] = useState<CarInfo | null>(null);
  ...
  const loadRow = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/car-infos/${id}`);
      if (!response.ok) {
  ...
  const handleSave = async (payload: CarInfo) => {
    if (!id) return;
    setIsSaving(true);
    try {
      const response = await apiPut(`/api/car-infos/${id}`, { ...payload, id });
  ...
  const handleDelete = async () => {
    if (!id) return;
    const response = await apiDelete(`/api/car-infos/${id}`);
    if (response.ok) {
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="border-b px-3 md:px-4">
          <div className="flex h-14 items-center justify-start gap-2">
            <Button type="submit" form={CAR_INFO_FORM_ID} disabled={isLoading || isSaving || !row} variant="outline" size="sm">
        ...
        {isLoading ? (
          ...
          row && <CarInfoForm formId={CAR_INFO_FORM_ID} initialCarInfo={row} isSubmitting={isSaving} onSubmit={handleSave} />
        )}
      </div>
