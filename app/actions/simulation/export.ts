...
const buildSimulationExportColumns = async (locale: string): Promise<CsvColumn<Simulation>[]> => {
  ...
  return [
    ...
    { label: tAdmin('columns.firstRegisteredAt'), format: (row) => formatDateOrDash(row.firstRegisteredAt) },
    ...
    { label: tAdmin('columns.created'), format: (row) => formatDateOrDash(row.createdAt, true) },
  ];
};
