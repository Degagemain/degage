...
function buildSimulationFromResult(
  ...
  return {
    ...
    resultMinSharedKm: result.resultMinSharedKm ?? null,
    resultAvgSharedKm: result.resultAvgSharedKm ?? null,
    resultMaxSharedKm: result.resultMaxSharedKm ?? null,
...
export async function createSimulation(input: SimulationRunInput, options?: { skipPersistence?: boolean }): Promise<Simulation> {
  ...
  captureEvent('simulation', { ...result, id: created.id });
  return created;
}
