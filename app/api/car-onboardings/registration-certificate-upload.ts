...
const parseUploadFile = async (request: NextRequest): Promise<RegistrationCertificateUploadFile | Response> => {
...
export const tryCarOnboardingRegistrationCertificateUpload = async (
  request: NextRequest,
  id: string,
  side: RegistrationCertificateSide,
  upload: (id: string, side: RegistrationCertificateSide, file: RegistrationCertificateUploadFile, user: UserWithRole) => Promise<void>,
  routeName: string,
  user: UserWithRole,
): Promise<Response> => {
  ...
  try {
    await upload(id, side, parsed, user);
    return noContentResponse();
  } catch (error) {
    if (error instanceof CarOnboardingLockedError || error instanceof CarOnboardingForbiddenError) {
      return forbiddenResponse(error.message);
    }
    if (error instanceof RegistrationCertificateNotRecognizedError) {
      return Response.json(
        { code: 'registration_certificate_not_recognized', errors: [{ message: error.message }] },
