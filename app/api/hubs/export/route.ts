...
export const GET = withAdmin(async (request: NextRequest) => {
  ...
  const filter = hubFilterSchema.safeParse(rawParams);
  if (!filter.success) {
  ...
  if (exportFormat === 'json') {
    return attachmentDownloadJsonResponse(JSON.stringify(await exportHubs(filter.data)), `hubs-${stamp}.json`);
  }

  return attachmentDownloadCsvResponse(await exportHubsCsv(filter.data), `hubs-${stamp}.csv`);
});
