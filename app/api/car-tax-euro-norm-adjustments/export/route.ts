...
import { exportCarTaxEuroNormAdjustments, exportCarTaxEuroNormAdjustmentsCsv } from '@/actions/car-tax-euro-norm-adjustment/export';
...
export const GET = withContext(async (request: NextRequest) => {
  ...
  const filter = carTaxEuroNormAdjustmentFilterSchema.safeParse(rawParams);
  if (!filter.success) {
  ...
  if (exportFormat === 'json') {
    return attachmentDownloadJsonResponse(
      JSON.stringify(await exportCarTaxEuroNormAdjustments(filter.data)),
      `car-tax-euro-norm-adjustments-${stamp}.json`,
    );
  }

  return attachmentDownloadCsvResponse(await exportCarTaxEuroNormAdjustmentsCsv(filter.data), `car-tax-euro-norm-adjustments-${stamp}.csv`);
});
