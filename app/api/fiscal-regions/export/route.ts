...
import { exportFiscalRegions, exportFiscalRegionsCsv } from '@/actions/fiscal-region/export';
...
export const GET = withContext(async (request: NextRequest) => {
  ...
  const filter = fiscalRegionFilterSchema.safeParse(rawParams);
  if (!filter.success) {
  ...
  if (exportFormat === 'json') {
    return attachmentDownloadJsonResponse(JSON.stringify(await exportFiscalRegions(filter.data)), `fiscal-regions-${stamp}.json`);
  }

  return attachmentDownloadCsvResponse(await exportFiscalRegionsCsv(filter.data), `fiscal-regions-${stamp}.csv`);
});
