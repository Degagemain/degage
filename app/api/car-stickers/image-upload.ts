...
import type { CarStickerImageUploadFile } from '@/actions/car-sticker/upload-image';
import { isPrismaNotFoundError, noContentResponse, notFoundResponse } from '@/api/utils';
import { statusCodes } from '@/api/status-codes';
import { logger } from '@/lib/logger';

const parseUploadFile = async (request: NextRequest): Promise<CarStickerImageUploadFile | Response> => {
...
export const tryCarStickerImageUpload = async (
  request: NextRequest,
  id: string,
  upload: (id: string, file: CarStickerImageUploadFile) => Promise<void>,
  routeName: string,
): Promise<Response> => {
