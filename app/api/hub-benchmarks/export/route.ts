...
import { exportHubBenchmarks, exportHubBenchmarksCsv } from '@/actions/hub-benchmark/export';
...
export const GET = withContext(async (request: NextRequest) => {
  ...
  const filter = hubBenchmarkFilterSchema.safeParse(rawParams);
  if (!filter.success) {
  ...
  if (exportFormat === 'json') {
    return attachmentDownloadJsonResponse(JSON.stringify(await exportHubBenchmarks(filter.data)), `hub-benchmarks-${stamp}.json`);
  }

  return attachmentDownloadCsvResponse(await exportHubBenchmarksCsv(filter.data), `hub-benchmarks-${stamp}.csv`);
});
