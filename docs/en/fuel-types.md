---
title: Fuel Types
roles:
  - admin
---

# Fuel Types

Reference list of fuel types (petrol, diesel, electric, etc.), used in the simulation, car infos, and car price estimates.

| Property         | Description                                                                       |
| ---------------- | --------------------------------------------------------------------------------- |
| Code             | Unique short identifier for the fuel type.                                        |
| Name             | Display name of the fuel type.                                                    |
| Order            | Sort position in selection lists (lower numbers first), including the simulation. |
| Active           | Whether the fuel type is available for selection.                                 |
| Price per        | Price-per-unit value used in calculations.                                        |
| CO2 contribution | CO2 contribution value used in calculations.                                      |

Use **New** next to the search box to add a fuel type, or open one from its name or the row **Edit** action to change it (including names per
language).

## Export

Use **More → Export** to download the current filtered/sorted list as CSV or JSON.

## Import

Use **More → Import** to upload a JSON file previously obtained via Export. Records with an id are updated; records without an id are inserted.
Each row shows its own status, and one failure does not stop the others.
